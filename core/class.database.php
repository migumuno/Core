<?php
	...
	class db{
	     ...
	     */
		public function query($query, $printQ = false){
		 ...
		 */
		public function stor($datos, $tabla, $pks = null, $printQ = false){
			switch ($this->dbtype){
	        	case 'mysql':
	        		return $this->mysql_stor($datos, $tabla, $pks, $printQ);
	        	break;
	        	case 'sqlsvr':
	        		...
	        		return $this->mssql_stor($datos, $tabla, $printQ);
	        	break;
	        }
		}
		 ...
		 */
		private function mysql_stor($datos, $tabla, $pks=null, $printQ = false){
			$get = new stdClass();
			$get->insert = false;
			$get->null	 = false;
			...
			//Hago el bind de los datos que realmente son los que me interesan de la tabla
			foreach($campos as $campo){
				if(in_array(":".$campo['COLUMN_NAME'], $campos_recibidos)){
					//añado los datos
					$this->bind(":".$campo['COLUMN_NAME'], $datos[":".$campo['COLUMN_NAME']]);
				}
			}
		 ...
		 */
		private function mssql_stor($datos, $tabla, $printQ = false){
			...
			//preparo la query
			$this->query($query, $printQ);
			
			//Hago el bind de los datos que realmente son los que me interesan de la tabla
			foreach($campos as $campo){
				if(in_array(":".$campo['name'], $campos_recibidos)){
					$this->bind(":".$campo['name'], $datos[":".$campo['name']]);
				}
			}
